...
app.post('/getQrCode', function (req, res,next) {
	...
	dbo.collection("testchecking-barcode").update(condition, addCollections, {multi: true},  function (err, obj) {
		if (err) throw err;
		...
		if (obj.result.n > 0){
			...
			dbo.collection("testchecking").find({qrCode: "0"}).toArray(function (err, result) {
				if (err) throw err;
				//đưa dữ liệu lấy được về JSON
				for(const i in result){
					let urlQrcode = `https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=${result[i]["_id"]}`;
					dbo.collection("testchecking-barcode").updateOne({_id: result[i]["_id"]}, {$set:{qrCode: urlQrcode}}, function (err, obj) {
...
app.post('/editStatus', function (req, res,next) {
	...
	if(idFromClient != ''){
		...
		dbo.collection("testchecking-barcode").updateOne(condition, editStatus,{upsert: false}, function (err, obj) {
			if (err) throw err;
			...
			//kiểm tra xem cập nhật thành công hay không obj.result.n >0 là thành công
			if (obj.result.n > 0){
				dbo.collection("testchecking-bar").findOne(condition, function (err, result) {
					if (err) throw err;
					//đưa dữ liệu lấy được về JSON
